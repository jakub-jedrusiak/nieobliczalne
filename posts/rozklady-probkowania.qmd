...
```{r}
tibble(group = c(rep("10", 10), rep("100", 100), rep("100000", 100000)), iq = map_vec(seq(length(group)), \(x) mean(rnorm(100, 100, 15)))) %>%
    mutate(group = factor(group, ordered = TRUE)) %>%
    ggplot(aes(iq, fill = group, colour = group, group = group)) +
    geom_histogram(aes(y = after_stat(count / sum(count))), alpha = 0.3, position = "identity") +
    geom_vline(xintercept = 100) +
    geom_vline(xintercept = c(98.5, 101.5), linetype = "dashed")
```
