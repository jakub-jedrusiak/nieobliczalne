...
theme_Publication <- theme_Publication() %+replace%
  theme(panel.background = element_rect(fill = "white"))

men <- readRDS("posts/dane/p-value/mezczyzni.RDS")
women <- readRDS("posts/dane/p-value/kobiety.RDS")
iq_means <- tibble(lp = 1:1000, IQ = readRDS("posts/dane/p-value/iq-means.RDS"))
iq_diffs <- readRDS("posts/dane/p-value/iq-diffs.RDS")
iq_stderr <- readRDS("posts/dane/p-value/iq-stderr.RDS")
```
...
iq_diffs %>%
  ...
  kbl(
    booktabs = TRUE,
    col.names = c("lp", "IQ kobiet", "IQ mężczyzn", "różnica")
  ) %>%
  kable_styling(latex_options = c("striped", "HOLD_position"))
```
...
::: {.callout-warning title="Pan Bayes protestuje"}
Ta logika jest nieintuicyjna -- dlaczego mamy odrzucać hipotezę zerową a nie potwierdzać alternatywną? Jaką mam pewność, że hipoteza alternatywna jest prawdziwa? A co jak mam porównać kilka hipotez? Na ile niemożność odrzucenia hipotezy jest dowodem na rzecz tej hipotezy?

Jakbym zamiast liczyć odchylenia od 0 liczyłbym odchylenia od 1 (co da się zrobić np. testem *t* dla pojedynczej próby), to tej hipotezy też nie mógłbym odrzucić, bo przy prawdziwej różnicy 1 też moglibyśmy takie dane łatwo uzyskać. To jest to, o czym tak naprawdę mówią tzw. przedziały ufności (*confidence intervals*) -- test *t* wyżej podaje „95% przedział ufności” [–6.17; 2.17], co oznacza, że równie dobrze nie można odrzucić hipotezy, że różnica wynosi 1, 0, –2 czy –6, ale która z nich jest najbardziej prawdopodobna? Jak bardzo jest prawdopodobna? W intuicyjnym myśleniu bardziej interesuje mnie, ile ta różnica rzeczywiście wynosi i z jakim prawdopodobieństwem, a nie ile nie wynosi! Często będziemy się zachowywać, jakby „brak możliwości odrzucenia hipotezy zerowej” był dowodem na jej prawdziwość, jak w przypadku testów normalności. Jednak na żadną z tych wątpliwości nasza statystyka nie odpowie.

Druga sprawa jest taka, że nasza logika może być tutaj wadliwa. Jak znam prawdziwe parametry rozkładu, to matematyka pozwala mi przewidzieć, jak będą się zachowywać próbki z tego rozkładu, na przykład że 95% próbek 2×100 osób da mi przypadkowe różnice z grubsza między –2 a +2. Ale w prawdziwym świecie ja nie znam prawdziwych parametrów rozkładu, chcę je dopiero poznać. Mam matematyczną gwarancję, że 95% próbek da mi wartości rozsądnie bliskie prawdziwym, ale czy to oznacza, że moja konkretna, pojedyncza próbka na 95% trafiła? Albo nawet że pomyliła się niewiele? Czy z tego, że 95% złodziei jest praworęcznych możemy wnioskować, że 95% praworęcznych to złodzieje? Bardzo często wyżej wspomniane przedziały ufności tłumaczy się tak, że na 95% prawdziwa średnia jest w tym przedziale, ale to nieprawda [@morey_fallacy_2016] -- *de facto* mówią nam tylko, że żadnej wartości z przedziału nie można odrzucić przy tych danych odrzucić przy $\alpha = 0,05$.

To jest rdzeń innej szkoły, która istnieje obok klasycznej statystyki (tzw. częstościowej) i nazywa się statystyką bayesowską. Opiera się ona o matematyczne twierdzenie, które na pewno pozwala odwracać prawdopodobieństwa poprawnie (tutaj -- pozwala policzyć prawdopodobieństwo prawdziwości hipotezy z prawdopodobieństwa, że stworzyłaby takie dane, jakie mamy), ale wymaga m.in. włączenia jakichś założeń *a priori* do naszego oszacowania. Bayesowskie wyciąganie wniosków to zupełnie inna filozofia, która jednak pozwala wprost porównywać hipotezy, określić prawdopodobieństwo prawdziwości danej hipotezy, a nawet wprost dostarczyć dowodów, że np. różnicy nie ma, a nie tylko, że nie możemy takiej ewentualności odrzucić. Pozwala też opracować tzw. przedziały wiarygodności (*credible intervals*), które naprawdę mają 95% pewności, że zawierają prawdziwą wartość parametru, czyli są tym, czym chcemy, żeby były przedziały ufności.

Statystyka bayesowska z roku na rok staje się popularniejsza, chociaż na standardowych studiach z psychologii ciężko ją spotkać. Sam dopiero się jej zaczynam na poważnie uczyć jako doktorant, w 2026 r., kiedy dopisuję tę ramkę. Zainteresowanym polecam swój nieopierzony wpis o [twierdzeniu Bayesa](twierdzenie-bayesa.qmd) oraz książkę *Bernoulli's Fallacy* [@clayton2021].
:::
