...
description: Wydział Nauk Historycznych i Pedagogicznych Uniwersytetu Wrocławskiego (wraz z innymi wydziałami) założył dziwny system drukarek, który nazywa się Printoscope. Namordowałem się strasznie próbując sprawić, żeby zadziałał z mojego własnego komputera, bo w Instytucie Psychologii mało kto korzysta z lokalnych komputerów. Piszę ten wpis, żeby udokumentować, czego się dowiedziałem. Co prawda dotyczy to tylko pracowników i doktorantów, ale to tym bardziej oznacza, że grono osób, którym chciałoby się z tym użerać i jeszce dzielić się doświadczeniem, jest wąskie. Oto więc instrukcja jak z prywatnego komputera podłączyć się do Printoscope WNHiP UWr.
...
Windows musi wiedzieć, z jakimi danymi podłączyć się do drukarki, bo liczba drukowanych kartek jest powiązana z osobą. Dlatego musimy go poinformować o naszych danych logowania. Niestety standardowy monit, który może się pojawić przy próbie dodania drukarki, wariuje w kontakcie z naszym uniwersytetem i jego zewnętrznym logowaniem, dlatego musimy go ominąć. W tym celu użyjemy Menedżera poświadczeń (*Credentials manager*). Wyszukujemy go wpisując jego nazwę w menu start lub znajdujemy w Panelu sterowania (w sekcji o użytkownikach). Po wejściu wybieramy *Poświadczenia systemu Windows* i dodajemy nowe poświadczenie. W polu na adres wpisujemy[^1]:
...
Nazwa użytkownika może być podchwytliwa, bo wymaga nazwy domeny i niekoniecznie jest tożsama z mailem. Doktoranci używają swojego numeru indeksu, podczas gdy pracownicy używają tajemniczego identyfikatora złożonego z pierwszej litery imienia i całego nazwiska, być może z jakimiś dodatkami. Nie wiem, jakie dokładnie reguły tym rządzą. Najpewniejszy sposób, na dowiedzenie się tego, to przyjście do wydziałowego budynku i zalogowanie się na którymś z lokalnych komputerów, tam otworzenie wiersza polecenia i użycie komendy `whoami`. Przypadkiem odkryłem też, że jeśli jesteśmy podłączeni do VPN i wejdziemy na stronę <http://printoscope.misc.local.uwr.edu.pl> (zwróćcie uwagę na http zamiast https), to nazwa użytkownika się wyświetla w komunikacie o błędzie. Niezależnie jaka ona jest, koniecznie trzeba ją poprzedzić członem `aduwr\`, np. `aduwr\jjedrusiak` lub `aduwr\315695`.
...
Panel Printoscope daje też możliwość logowania się do drukarek kartą pracowniczą zamiast hasłem czy PIN. Nie mam takiej karty, ale z ciekawości spróbowałem zamiast karty pracowniczej dodać swoją legitymację doktorancką i inne karty zbliżeniowe. W tym celu pobrałem aplikację [NFC Tools](https://play.google.com/store/apps/details?id=com.wakdev.wdnfc&hl=pl) i przyłożyłem do telefonu swoją legitymację. Uzyskałem w ten sposób numer seryjny, który przepisałem do Printoscope bez dwukropków (czyli jeśli numer seryjny byłby 12:34:56:78, to w Printoscope wpisałem 12345678). Po dodaniu podszedłem do drukarki, przyłożyłem legitymację w odpowiednim miejscu i rzeczywiście to zadziałało! Teraz legitymacja zastępuje mi PIN czy hasło. Muszę tylko zabrać kartę od czytnika dość szybko, po pierwszym piśnięciu drukarki, bo inaczej wylogowuje. Spróbowałem tego samego z dowodem osobistym i kartą bankową, ale te niestety (albo stety) nie zadziałały. Wygląda na to, że muszą to być proste karty zbliżeniowe typu właśnie legitymacja, karta parkingowa, brelok do domofonu, może Multisport albo Urbancard. Warto przetestować. Jeśli NFC Tools po przyłożeniu karty pokazują rodzaj taga `Mifare Classic 1k` lub standard `ISO 14443A`, to powinno zadziałać. Inne trzeba po prostu sprawdzić, a jak komuś zależy, to [czysta karta zbliżeniowa](https://allegro.pl/listing?string=mifare%20classic%201k&order=d) kosztuje 2-6 zł.
...
::: {#resolvectl .callout-warning}
## Problemy z `systemd-resolved` w Arch

W Arch Linux (i poddystrybucjach typu Manjaro) czasem VPN może działać losowo -- czasem tak, czasem nie. To wynika z konfiguracji DNS. Jeśli są z tym problemy, to warto jest otworzyć konfigurację NetworkManagera:

``` bash
sudo nano /etc/NetworkManager/NetworkManager.conf
```

Dodać tam lub zamienić sekcję:
...
Następnie uruchomić `systemd-resolved`:

``` bash
sudo systemctl enable --now systemd-resolved
```

I podłączyć pliki konfiguracyjne:

``` bash
sudo ln -sf /run/systemd/resolve/stub-resolv.conf /etc/resolv.conf
```

Teraz powinno działać konsekwentnie.
:::

## Dodanie poświadczeń

To jest ten bardziej irytujący moment. Samba w Windowsie potrafi sama znaleźć `\\printsrv-wnhp`, jednak Linux potrzebuje rzeczywistego adresu IP. Nie wiem, czy wolno jest mi tutaj go podać, więc na wszelki wypadek opiszę tylko metodę, jak go znaleźć (choć zawsze można do mnie napisać). Niestety potrzebujemy do tego podłączonego do VPN UWr komputera z Windowsem lub maszynę wirtualną (z zainstalowanym GlobalProtect). Może być jeden z komputerów na miejscu, to będzie można przy okazji upewnić się co do nazwy użytkownika. By poznać prawdziwy adres IP tej lokalizacji sieciowej, musimy w wiersz polecenia (nie PowerShell, a zwykłe cmd) wpisać następującą komendę:
...
Oczywiście podstawiamy nasz login (już bez `aduwr\`, bo to jest już przed średnikiem), poznany wcześniej adres IP i nazwę pliku PPD. Po dodaniu przestawiamy domyślny rozmiar papieru na A4 i tryb na zwykły (żeby nie było zawsze czarno-biało). Najłatwiej zrobić to w systemowym graficznym menu drukarek. Moja komenda (ze zmienionym dla bezpieczeństwa IP) wyglądała ostatecznie tak:
