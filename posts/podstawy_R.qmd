...
image: ./headers/podstawy_R.webp
twitter-card:
    image: "/headers/podstawy_R.jpeg"
freeze: true
categories:
    - Statystyka
    - R
...
[^podstawy_r-10]: Jeśli nie wiesz o co chodzi, nie przejmuj się. To duży temat i nie jest to coś, bez czego nie da się żyć. W skrócie to system do monitorowania historii zmian w plikach i współpracy nad kodem. Więcej o tym piszę [tutaj](./posts/git.qmd).
...
df_scream <- read_csv(
  "https://raw.githubusercontent.com/profandyfield/discovr/master/data-raw/csv_files/escape.csv"
)
...
db_retest <- read_csv(
  "./dane/podstawy-R/join-retest.csv",
...
Współczesnym narzędziem do utrzymywania porządku w kodzie R jest [Air](https://posit-dev.github.io/air/). To proste narzędzie zdejmuej z nas potrzebę myślenia o wyglądzie naszego kodu. Dobrze ustawiony Air sprawia, że za każdym razem, gdy zapisujemy nasz plik, kod jest automatycznie formatowany w zgodzie ze standardami. By skorzystać z Air, musimy pobrać go używając terminala. Możemy to zrobić bezpośrednio w RStudio, przełączając się z zakładki `Console` na zakładkę `Terminal`. W to okno należy wkleić komendę, którą znajdziemy [na stronie Air](https://posit-dev.github.io/air/cli.html). Teraz należy wyłączyć i włączyć RStudio, by pewne zmienne systemowe mogły się odświeżyć. Air jest już zainstalowany i dostępny. By podłączyć Air do Rstudio, musimy dowiedzieć się, gdzie konkretnie został on zainstalowany. Dowiemy się tego znowu korzystając z zakłdaki `Terminal`. Na Windowsie wykorzystujemy komendę `where air`, zaś na Linux/MacOS `which air`. Po kliknięciu Enter, wyświetli nam się dokładna ścieżka do Air. Kopiujemy ją. Następnie wchodzimy w ustawienia RStudio, Tools → Global Options → Code i zakładka Formatting. Tam z rozwijanej listy przy "Code formatter" wybieramy "External". Wyświetli nam się możliwość ustawienia "Reformat command". Wklejamy tam wcześniej skopiowaną ścieżkę do Air i po spacji dopisujemy słowo "format", np. ostatecznie uzykamy `/home/jakub/.local/bin/air format`. Co ważne, jeśli w ścieżce macie jakiekolwiek spacje, musicie ująć ją w podwójne cudzysłowy, np. `"/home/jakub jedrusiak/.local/bin/air" format`. Wreszcie przełączamy się do zakładki "Saving" i zaznaczamy opcję "Reformat documents on save". Gotowe! Teraz za każdym razem, gdy zapisujemy plik, staje się on ładny, czytelny i przede wszystkim spójny.

## `lintr` {#lintr}

`Air` nie poprawia wszystkich błędów, bo byłoby to zbyt destrukcyjne. Na szczęście `lintr` może nam większość z nich wypomnieć. Dla każdego naszego projektu powinniśmy raz uruchomić komendę `lintr::use_lintr(type = "tidyverse")`. Spowoduje to wytworzenie w folderze pliku o nazwie `.lintr`. Zawiera on konfigurację `lintr` dla danego projektu. Za jego pomocą będziemy mogli tę konfigurację również zmieniać.
...
df_raw <- read_csv(
  "https://raw.githubusercontent.com/jakub-jedrusiak/jakub-jedrusiak.github.io/main/statystyka/dane/podstawy-R/complex_database.csv"
)

df_KTR_test <- read_csv(
  "https://raw.githubusercontent.com/jakub-jedrusiak/jakub-jedrusiak.github.io/main/statystyka/dane/podstawy-R/join-test.csv"
)

df_KTR_retest <- read_csv(
  "[./dane/podstawy-R/join-retest.csv](https://raw.githubusercontent.com/jakub-jedrusiak/jakub-jedrusiak.github.io/main/statystyka/dane/podstawy-R/join-retest.csv)"
)
