...
Po pierwsze, musimy ustalć, od jakiej daty chcemy zacząć. Możemy wykorzystać dzisiaj wpisując `datetime.date.today()`. Możemy też wybrać datę początkową arbitralnie, wpisując `datetime.date(2023, 5, 11)`. Data jest w formacie ISO 8601, czyli rok, miesiąc, dzień.

Po drugie będziemy musieli dodawać do naszej daty dni. Robimy to funkcją `datetime.timedelta(days = 1)`. W tej formie dodajemy 1 dzień.

Po trzecie nasza data musi być w określonym formacie, w tym wypadku DD.MM.RRRR. Domyślnie daty wyświetlają się w formacie ISO 8601, czyli RRRR-MM-DD. Formatować daty można metodą (szczególny rodzaj funkcji) `strftime`. Jak to rzeczywiście zastosować, najlepiej będzie pokazać w przykładzie. Do samej metody wrzucamy format, w jakim datę chcemy uzyskać. Wykorzystamy tutaj kody, których listę możemy znaleźć [tutaj](https://www.w3schools.com/python/gloss_python_date_format_codes.asp). Nasz format zakodujemy jako `"%d.%m.%Y"`.
...
...i to też zadziała. Zależy co uznamy za bardziej czytelne. Pamiętamy przy tym, że domyślnie `range(7)` naturalnie generuje liczby od 0 do 6, więc na początku `timedelta()` dodaje 0 dni, potem 1 dzień i aż do 6 dni. Uzyskujemy więc tydzień. Siła automatyzacji polega na tym, że mając te 4 linijki kodu, taki sam nakład pracy potrzebny do zrobienia takiej rozpiski dla tygodnia, miesiąca czy 30 lat.

# Podsumowanie

Opanowanie automatyzacji wymaga sporo praktyki i początkowo zajmować więcej czasu, niż wykonanie jakiejś pracy ręcznie. Jednak z doświadczeniem przychodzi efektywność. Jednak już opanowanie podstaw sprawia, że często możemy oszczędzić sobie wielu, wielu godzin pracy. Podsumujmy to.
...
7.  Zmienne z datami tworzymy za pomocą `date()`, różnice w datach liczymy przez `timedelta()` oraz możemy formatować daty poprzez metodę `strftime()`.
